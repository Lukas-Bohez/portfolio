  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <section className="mb-8 rounded-3xl border border-surface bg-surface p-6 shadow-lg sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Separate CMS Demo</p>
        <h1 className="mt-3 text-3xl font-extrabold text-primary sm:text-4xl">Next.js + Sanity + Cloudinary PoC</h1>
        <p className="mt-3 max-w-3xl text-base text-default sm:text-lg">
          This route is isolated from the regular portfolio landing page. Content below is loaded in a Server Component from Sanity, and project images are expected as Cloudinary URLs.
        </p>
      ...
      <section className="mb-8 grid gap-4 sm:grid-cols-2">
        <article className="rounded-2xl border border-surface bg-surface p-5 shadow-md">
          <p className="text-xs font-bold uppercase tracking-[0.15em] text-primary">Caching Strategy</p>
          <p className="mt-2 text-sm text-default">
            Project list uses server-side fetch with <strong>revalidate: 3600</strong> because this content changes rarely.
          </p>
        </article>
        <article className="rounded-2xl border border-surface bg-surface p-5 shadow-md">
          <p className="text-xs font-bold uppercase tracking-[0.15em] text-primary">Dynamic Strategy</p>
          <p className="mt-2 text-sm text-default">
            Profile text uses a shorter fetch window (<strong>revalidate: 300</strong>) so edits appear quickly while keeping route performance stable.
          </p>
        </article>
      </section>
      ...
      <section className="mb-8 rounded-3xl border border-surface bg-surface p-6 shadow-lg sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Recruiter + Investor Signals</p>
        <h2 className="mt-3 text-2xl font-bold text-primary sm:text-3xl">Why this portfolio is built to stand out</h2>
        <p className="mt-2 max-w-3xl text-sm text-muted sm:text-base">
          This demo is intentionally optimized for rapid iteration, measurable delivery, and polished storytelling, three qualities that matter in recruiter screens,
          investor conversations, and innovation tracks like Red Bull Basement.
        </p>

        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          <article className="rounded-2xl border border-blue-400/40 bg-blue-400/10 p-4 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">Execution Speed</p>
            <p className="mt-2 text-sm text-default">Release-linked portfolio updates and deployment verification are integrated directly into the workflow.</p>
          </article>
          <article className="rounded-2xl border border-emerald-400/40 bg-emerald-400/10 p-4 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">Product Clarity</p>
            <p className="mt-2 text-sm text-default">Content model, technical stack, and user-facing outcomes are visible in one clean narrative.</p>
          </article>
          <article className="rounded-2xl border border-amber-400/40 bg-amber-400/10 p-4 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">Innovation Readiness</p>
            <p className="mt-2 text-sm text-default">Cloud media, motion-rich UI, and CMS-driven updates support fast prototyping and public demo quality.</p>
          </article>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-surface bg-surface p-6 shadow-lg sm:p-8">
        ...
        <p className="mt-2 text-sm text-muted">
          Edit project documents in Sanity and refresh this page to see updates without modifying code.
        </p>

        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          {projects.map((project) => {
            return (
              <article key={project._id} className="overflow-hidden rounded-2xl border border-surface bg-surface shadow-md">
                <div className="p-5">
      ...
      <section className="cms-cloud-gallery mt-10 rounded-3xl border border-surface bg-surface p-6 shadow-lg sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Cloudinary Gallery</p>
        <h2 className="mt-3 text-2xl font-bold text-primary sm:text-3xl">Cloud-loaded visual examples</h2>
        ...
        <div className="cms-gallery-marquee mt-6" aria-label="Scrolling cloud image gallery">
          <div className="cms-gallery-track">
            {[...cloudGalleryImages, ...cloudGalleryImages].map((image, index) => {
              const transformedUrl = withCloudinaryTransform(image.src, 'f_auto,q_auto,w_900,h_540,c_fill,g_auto');
              return (
                <figure key={`${image.id}-row-a-${index}`} className="cms-gallery-card">
                  <Image
                    src={transformedUrl}
                    alt={image.alt}
                    width={900}
        ...
        <div className="cms-gallery-marquee cms-gallery-marquee--reverse mt-4" aria-hidden="true">
          <div className="cms-gallery-track">
            {[...cloudGalleryImages.slice().reverse(), ...cloudGalleryImages.slice().reverse()].map((image, index) => {
              const transformedUrl = withCloudinaryTransform(image.src, 'f_auto,q_auto,w_900,h_540,c_fill,g_auto');
              return (
                <figure key={`${image.id}-row-b-${index}`} className="cms-gallery-card cms-gallery-card--compact">
                  <Image
                    src={transformedUrl}
                    alt={image.alt}
                    width={900}
